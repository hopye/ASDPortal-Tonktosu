...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'communication-symbols',
        name: 'communication-symbols',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["image/jpeg", "image/png", "image/jpg", "image/webp", "image/svg+xml"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for communication-symbols" ON storage.objects FOR SELECT USING (bucket_id = 'communication-symbols');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for communication-symbols" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'communication-symbols');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for communication-symbols" ON storage.objects FOR UPDATE USING (bucket_id = 'communication-symbols');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for communication-symbols" ON storage.objects FOR DELETE USING (bucket_id = 'communication-symbols');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'communication-symbols',
        public: true,
        allowed_mime_types: ["image/jpeg", "image/png", "image/jpg", "image/webp", "image/svg+xml"],
